...
function newuserUrl(id) {
  return `${window.location.origin}/newuser?id=${encodeURIComponent(id)}`;
}
...
function showQrImage(image, downloadBtn, link) {
  const qrApi =
  ...
  return new Promise((resolve, reject) => {
    image.onload = () => {
      ...
      downloadBtn.download = `petty-${Date.now()}.png`;
      resolve();
    };
...
function setupAdminQr() {
  ...
  async function generateQr() {
    showError("");
    const id = makeTagId();
    const link = newuserUrl(id);

    try {
      await showQrImage(image, downloadBtn, link);
...
function setupUserPage() {
  ...
  getTag(id)
    .then((tag) => {
      if (!tag) {
        window.location.replace(newuserUrl(id));
        return;
      }

      document.getElementById("display-pet-name").textContent = tag.petName;
      document.getElementById("display-owner-name").textContent = tag.ownerName;
      document.getElementById("display-pet").textContent = tag.petName;

      const phoneEl = document.getElementById("display-phone");
      phoneEl.textContent = tag.phone;
      phoneEl.href = `tel:${tag.phone}`;
    })
    .catch(() => {
      window.location.replace(newuserUrl(id));
    });
...
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("generate-btn") && document.getElementById("qr-image")) {
    setupAdminQr();
    setupNavScroll();
    return;
  }
